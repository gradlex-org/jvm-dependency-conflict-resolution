...
public abstract class LWJGLTargetPlatformRule implements ComponentMetadataRule {
    ...
    public void execute(ComponentMetadataContext context) {
        ...
        String dependency = details.getId().getGroup() + ":" + details.getId().getName();
        for (LWJGLTarget target : TARGETS) {
            addTargetPlatformVariant(details, target);
        }
        allVariantsMatching(
                context,
                id -> id.matches(Usage.USAGE_ATTRIBUTE, Usage.JAVA_RUNTIME)
                        && id.matches(Category.CATEGORY_ATTRIBUTE, Category.LIBRARY),
                v -> v.withDependencies(
                        dependencies -> dependencies.add(dependency, d -> addCapability(d, dependency + "-natives"))));
    }

    private void addTargetPlatformVariant(ComponentMetadataDetails details, LWJGLTarget target) {
        String group = details.getId().getGroup();
        String name = details.getId().getName();
        String version = details.getId().getVersion();

        details.addVariant(target.classifier + "Runtime", "runtime", variant -> {
            variant.withCapabilities(c -> {
                c.removeCapability(group, name);
                c.addCapability(group, name + "-natives", version);
            });
