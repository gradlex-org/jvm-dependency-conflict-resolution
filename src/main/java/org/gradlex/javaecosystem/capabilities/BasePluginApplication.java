...
final class BasePluginApplication {
            ...
            case FAIL_ON_PROJECT_RULES:
                if (!baseAppliedViaSettings) {
                    throw new IllegalStateException(
                            "RulesMode is set to " + rulesMode + " in " + settingsFileName + " but the '" + JavaEcosystemCapabilitiesBasePlugin.PLUGIN_ID + "' plugin was not applied via settings." +
                                    " As a result this plugin will not work." +
                                    " Fix this problem by either changing dependencyResolutionManagement.rulesMode to PREFER_PROJECT or by applying '" + JavaEcosystemCapabilitiesBasePlugin.PLUGIN_ID + "' as a settings plugin in " + settingsFileName + "."
                    );
                }
                break;
            default:
    ...
    private boolean isBaseAppliedViaSettings() {
        return getSettings().getPlugins().hasPlugin(JavaEcosystemCapabilitiesBasePlugin.class);
    }
