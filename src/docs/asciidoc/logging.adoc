...
It is built on the https://docs.gradle.org/6.0.1/userguide/component_capabilities.html[Gradle capabilities feature], to select a Java logger implementation so that it is enforced at build time!
...
The application will make sure that the <<rules.adoc#capabilities,capabilities definition plugin>> has been applied as well, either as a settings or project plugin.
